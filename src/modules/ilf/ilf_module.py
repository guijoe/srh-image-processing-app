...
class GaussianBlurParamsWidget(QWidget):
    def __init__(self, parent=None):
        ...
        # We use a slider mapped to a float value for Sigma (Blur Intensity)
        self.label = QLabel("Blur Intensity (Sigma: 1.0):")
        layout.addWidget(self.label)
        self.slider = QSlider(Qt.Horizontal)
        self.slider.setRange(5, 50) # Represents 0.5 to 5.0
        self.slider.setValue(10)     # Represents 1.0
...
class SharpenParamsWidget(QWidget):
    def __init__(self, parent=None):
        ...
        layout.addWidget(self.label)
        self.slider = QSlider(Qt.Horizontal)
        self.slider.setRange(1, 30) # Represents 0.1 to 3.0
        self.slider.setValue(10)    # Represents 1.0
        self.slider.valueChanged.connect(self._on_value_changed)
        layout.addWidget(self.slider)
...
class ILFControlsWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        # Mapping operations to their new dynamic UI parameter widgets
        operations = {
...
class IlfImageModule(IImageModule):
    ...
    def _manual_convolve(self, image: np.ndarray, kernel: np.ndarray) -> np.ndarray:
        """Hand-written 2D convolution using NumPy for academic demonstration."""
    ...
    def process_image(self, image_data: np.ndarray, metadata: dict, params: dict) -> np.ndarray:
        ...
        if op == "Brightness Adjustment":
            ...
            # Create a 5x5 Gaussian Kernel programmatically using the Sigma parameter
            size = 5
            ...
            kernel /= np.sum(kernel) # Normalize kernel sum to 1.0
            
            processed = self._manual_convolve(img, kernel)

        elif op == "Adjustable Sharpen Filter":
            factor = params.get('factor', 1.0)
            # Center value increases with strength factor while subtraction values stay constant
            ...
            # Apply user's custom threshold choice
            if threshold > 0:
